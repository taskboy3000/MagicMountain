...
subtest 'columns extend defaults with name, account_id, season_id, score, scrap, turns_remaining, pending_activity_id' => sub {
    is_deeply(
        $char->columns,
        [qw{id updatedAt createdAt name account_id season_id score scrap turns_remaining pending_activity_id}],
        'Character columns include name, account_id, season_id, score, scrap, turns_remaining, pending_activity_id'
    );
};
...
subtest 'create - valid Character columns' => sub {
    ...
    is($obj->row->{turns_remaining}, 7,           'create sets turns_remaining');
};

subtest 'getCol / setCol on subclass columns' => sub {
    my $obj = $char->create(name => 'Elinor', account_id => 'a1', season_id => 's1', score => 10, scrap => 5, turns_remaining => 3);
    ...
    is($obj->getCol('turns_remaining'), 3, 'getCol returns turns_remaining');
    $obj->setCol('score', 99);
